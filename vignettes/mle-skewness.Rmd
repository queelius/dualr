...
derivatives unlock additional diagnostic information — in particular, the
...
We observe $x_1, \ldots, x_n \sim \text{Gamma}(\alpha, \beta)$ with shape
$\alpha > 0$ and rate $\beta > 0$. The log-likelihood, written in terms of
...
This model is ideal for demonstrating third-order AD because:
...
We use `optim` with AD gradients to find the MLE:
...
The real payoff of `D(f, x, order = 3)` is the third-order derivative
tensor $T_{rst} = \partial^3 \ell / \partial\theta_r\partial\theta_s\partial\theta_t$.
For a two-parameter model, this is a $2 \times 2 \times 2$ array with 8
entries (though symmetry reduces the distinct values).
...
We validate the theoretical skewness by simulating many datasets and
...
The histograms show the sampling distribution of the MLEs, with the
asymmetric shape predicted by the skewness analysis:
