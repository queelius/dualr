...
ll_normal <- function(theta) {
  mu <- theta[1]
  sigma <- exp(theta[2])  # unconstrained: theta[2] = log(sigma)
  -n * log(sigma) - (1 / (2 * sigma^2)) * (sum_x2 - 2 * mu * sum_x + n * mu^2)
}
```

**Reparameterization.** Because $\sigma > 0$, we optimize over
$\theta_2 = \log\sigma$ so that the parameter space is fully unconstrained —
a standard practice in MLE fitting. Without this, BFGS can step $\sigma$
through zero, producing `NaN` from `log(sigma)` and causing divergence.
The AD chain rule propagates through `exp()` automatically, so `score()` and
...
# Compare (transform theta[2] back to sigma for display)
data.frame(
